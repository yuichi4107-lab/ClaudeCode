...
| file_path | TEXT | NOT NULL | サーバー上の保存パス(`data/attachments/{order_no}/`) |
...
- SQLite ファイルと `data/attachments/` を日次でバックアップディレクトリへコピー(7世代以上保持)
- WALモードで運用し、バックアップは `VACUUM INTO`(または `.backup`)で整合性を保って取得する
