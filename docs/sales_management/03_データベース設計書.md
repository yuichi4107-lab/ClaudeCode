...
- **DBファイルの配置**: アプリ稼働PC(常時起動PC)のローカルディスクに置く。**NAS共有フォルダに直接配置して複数PCからSMB越しに開くことは禁止**(ネットワーク共有上ではファイルロックが機能せず破損リスクがあるため)。共有フォルダへはバックアップ(4章)として日次コピーする
...
erDiagram
    ...
    invoices {
        ...
        integer customer_id FK
        text billing_month "対象月(YYYY-MM)"
        date issue_date "請求日"
...
共通: 全テーブルに `created_at`(作成日時)、`updated_at`(更新日時)、主要テーブル(sales_orders / purchase_orders / deliveries / invoices)には `created_by` / `updated_by`(users.id)を持たせる(ER図では省略)。
...
| customer_id | INTEGER | NOT NULL FK→customers.id | |
| billing_month | TEXT | NOT NULL | 対象月 `YYYY-MM` |
...
- 形式: `INV-YYYYMM-NNN`(例: `INV-202607-001`)。対象月内の連番
...
- アプリ稼働PCの故障時は、最新バックアップDBを新しいPCに配置してアプリを再セットアップすれば復旧できる(復旧手順は実装フェーズで運用手順書として整備)
