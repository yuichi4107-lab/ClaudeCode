...
> ⚠️ **このドキュメントはv1（GitHub Actions 方式）の手順です。**
...
[
  {
    ...
    "image_prompt": "NanoBanana2に渡すプロンプト（画像不要なよnull）",
    "image_path": null,
...
or なし）
  ↓
queue.json から削除 → コミットープッシュ
```

## 手動テスト方法

GitHub の Actions タブ → 「X自動投稿」→「Run workflow」
- `dry-run: true` にすると投稿せずて確認してください。

## キューが空のとき

投稿はスキップされます（エラーにはなりません）。
キューが空になる前に Claude Code でストックを補入してください。
