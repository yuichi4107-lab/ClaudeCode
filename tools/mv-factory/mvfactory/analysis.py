...
軽量な実装方針(librosa等の重い依存を避ける、要件定義書の指示どおり):
  ...
  - セクション区切り: 歌詞テキストの空行区切り�雙用)を�*��)
    壚元でに使う。歌詞に空行区至がなければ、堺を嗯等分刲してセクシ・ンを作る。
  - Whisperタイムスタンプ: 手持ち音準にボーカルがある場合、short-video-editor
    スキルと同じ Whisper API 呼び出しパターンを流用可能(OPENAI_API_KEY要)。
    本モジュールはWhisper連掚をオプション機能として提供し、キー未設定/
    呼び出失敗時はギーヮニヮギーマクーバックする（解析誤差時のフォールバック方針)。
