...
この文書は、READMEや過去の引き継ぎのだけでなく、2026-07-27時点のソースコード、テスト、launchd設定、読み取り専用のライブ状態を照合して作成した現状仕様である。
...
- プロセス間ロックがないため、同時runが同じ承認済行を選ぶ禮合を防げない。
...
- ただし、このスナップショットは、49商品のURL一対一照合までは行っていない。
