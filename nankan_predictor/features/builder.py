...
from nankan_predictor.config.settings import (
    VENUE_CODES,
    TRACK_CONDITION_ENC,
    TRACK_TYPE_ENC,
)
...
class FeatureBuilder:
    ...
    def build_training_set(
        ...
        Returns (X, y):
          X: 特徴量行列（1行=1出走）
          y: ラベル。target="win" -> 1着(is_winner), target="place" -> 2着(is_second)
        """
    ...
    def _jockey_features(self, entry: pd.Series) -> dict:
        ...
        # このコンビの過去勝利数は race_entries を直接参照するのが正確だが
        # シンプルにするため history から近似する
        pair_wins = np.nan
    ...
    def _race_features(self, entry: pd.Series) -> dict:
        venue_code = entry.get("venue_code", "")
        venue_enc = list(VENUE_CODES.values()).index(venue_code) if venue_code in VENUE_CODES.values() else np.nan
