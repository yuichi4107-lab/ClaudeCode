...
# --- 12番人気データ ---
# 12番人気が1着で来た回数: 11回 (10.5%)
POP12_WIN_COUNT = 11

# --- 1-2人気馬単が全く来ない日 ---
# 1日3レッグのうち1-2人気の組み合わせが来る日: 14日 (40%)
# → 来ない日: 21日 (60%)
DAYS_WITH_FAV_EXACTA = 14
DAYS_WITHOUT_FAV_EXACTA = TOTAL_DAYS - DAYS_WITH_FAV_EXACTA

# --- 非人気1着の人気帯 ---
# 12番人気以外は3〜9番人気 (10,11番人気が1着のときは相手が1番人気)
# → 1着が荒れても3〜9番人気の範囲が中心
...
def main():
  ...
  │                                                         │
  │ 12番人気が1着: {POP12_WIN_COUNT}/{TOTAL_RACES} = {pct(POP12_WIN_COUNT, TOTAL_RACES):.1f}% (★多い!)  │
  └─────────────────────────────────────────────────────────┘
""")
  ...
  パターン         理論値      推定日数(/35)  含意
  ─────────────────────────────────────────────────────────
  3レッグとも堅い    {p_all3*100:5.1f}%      {p_all3*TOTAL_DAYS:.0f}日       堅い日 → 低配当
  2レッグ堅い       {p_2of3*100:5.1f}%      {p_2of3*TOTAL_DAYS:.0f}日       ★最も多い典型パターン
...
""")
  ...
  【非人気1着の人気帯】
   ...
   ・12番人気が1着: {POP12_WIN_COUNT}回 ({pct(POP12_WIN_COUNT, TOTAL_RACES):.1f}%) ★想定以上に多い
""")
    ...
    # B: 3-9人気が1着 (約22%) → 「中穴パターン」
    # C: 10-12人気が1着 (約10%) → 「大穴パターン」
...
""")
    ...
    # ============================================================
    print("─" * 80)
    print("■ 6. 12番人気問題（大穴対策）")
    print("─" * 80)

    print(f"""
  12番人気が1着に来た回数: {POP12_WIN_COUNT}回/{TOTAL_RACES}レース = {pct(POP12_WIN_COUNT, TOTAL_RACES):.1f}%

  これは理論値（Harvilleモデルで約0.5%）の約20倍!
  地方競馬では出走取消・実力差が大きい場合に大穴が頻発する。

  【対策】
  ・12番人気を全レッグにいれると点数が爆発する → 非現実的
  ・10-11番人気が1着のとき2着は1番人気が多い
  ・大穴は「来たらラッキー」程度にランダム枠で対応

  【推奨】
  ・3レッグのうち最も荒れそうな1レッグだけ、10-12番人気を追加
  ・「一部ランダム」でカバーするのが効率的
  ・フル自力予想では12番人気はカバーし切れない
""")
  ...
  │      (10番人気以降はランダム枠でカバー)                        │
  │                                                              │
  │ 5. 12番人気の1着が10.5%もある(理論値の20倍)                   │
  │    → 全レッグには入れられないが、最も荒れそうなレッグだけ追加    │
  │    → SPAT4の「一部ランダム」機能が有効                        │
  │                                                              │
